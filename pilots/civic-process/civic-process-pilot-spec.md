...
A clean conceptual separation is essential here, because the existing [Civic Process Specification](../../ecosystem/civic-process-spec.md) §6.1 describes a process *descriptor*, which is sometimes confused with a plugin *manifest*. They are different objects answering different questions.
...
The **Civic Process Descriptor** (defined in the [Civic Process Specification](../../ecosystem/civic-process-spec.md) §6.1) is a JSON object describing what a process *is*: its type, title, jurisdiction, lifecycle configuration, action schemas, and eligibility requirements. Every process publishes its descriptor at `GET /process/:id`; the hub's full process catalog is available at `GET /process`. This is the static, structural side of the export.
