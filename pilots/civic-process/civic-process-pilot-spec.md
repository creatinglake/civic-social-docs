...
- **[Civic Process Specification](../../specs/civic-process-spec.md)** — the protocol-level definition of a Civic Process: lifecycle, identity requirements, event contracts, and action contracts.
...
- **[Civic Activity Specification](../../specs/civic-activity-spec.md)** — the activity/event distribution layer that every plugin must conform to.
...
The pilot is foundational, not platform-specific. It is the work that turns the [Civic Process Specification](../../specs/civic-process-spec.md) and the [Civic Plugin Architecture](../../ecosystem/civic-plugin-architecture-spec.md) into shippable infrastructure: a reference implementation of the framework, three plugin exemplars that exercise it, and the public specification that any future plugin author can build against.
...
The Civic.Social architecture is organized around **four canonical open specifications** — the Civic Space Specification, the Civic Process Specification, the Civic Activity Specification, and the Civic Identity Specification (the Civic Hub is a community-scoped Civic Space type defined by the Civic Space Specification) — each of which extends underlying open web standards (W3C DIDs and Verifiable Credentials, ActivityPub, ActivityStreams, among others) into a civic-participation context. Together those four specs form the interoperability foundation that the rest of the ecosystem — citizen dashboards, activity feeds, discovery interfaces, representative spaces, and any number of third-party civic applications — builds on as interfaces and downstream tools, not as foundational components. The plugin framework defined in this document operates at the **Civic Process Specification** layer: it is the work that turns the Civic Process Specification into installable, portable, interoperable participation tools.

The plugin framework sits between the Civic Process Specification (which defines what a process *is*) and the host environments that run processes (Civic Hubs, citizen dashboards, representative spaces, external embeds). It is the translation layer that lets a single process implementation run across many hosts without per-host integration work. It is also the integration point with the Civic Identity Specification (plugins consume identity from the shared layer rather than managing their own) and with the Civic Activity Specification (plugins emit activities into the shared distribution layer).

The plugin framework is a keystone of the Civic.Social interoperability story. The Civic Identity Specification makes citizens portable across the ecosystem; the Civic Activity Specification makes participation visible everywhere it happens; the Civic Space Specification makes the spaces where participation occurs interchangeable; and the plugin framework — built on the Civic Process Specification — makes the *processes themselves* portable. Each of the other three canonical specs reaches its full value only when plugins are also standardized: without portable plugins, the Civic Identity Specification becomes single-sign-on to a small number of disconnected tools rather than a credential that works across an open ecosystem; the Civic Activity Specification becomes aggregation across siloed sources rather than a coherent stream of participation; and the Civic Space Specification gives communities interchangeable spaces that still require bespoke per-process integration to host anything inside them.
...
A clean conceptual separation is essential here, because the existing [Civic Process Specification](../../specs/civic-process-spec.md) section 6.1 describes a process *descriptor*, which is sometimes confused with a plugin *manifest*. They are different objects answering different questions.
...
The capability declaration schema now exists as a machine-readable JSON schema in the [Civic Plugin Architecture](../../ecosystem/civic-plugin-architecture-spec.md) §4.1; the pilot requires every plugin manifest to declare its capabilities against it. Enforcement follows the staged path defined in the Civic Plugin Architecture §5a: **Stage 1 host-seam enforcement is in scope for this pilot now** — the activity emission path validates every emission against the plugin's declared types, and the identity adapter filters participant context to the plugin's granted identity claims before handoff. The Stage 2 sandbox (which makes capabilities mechanically enforceable for Tier 2 code) remains deferred, and that deferral is bounded by a normative requirement of the plugin architecture: **a host MUST NOT run third-party plugin code in production civic processes before Stage 2 enforcement exists for that code's tier** (§5a). Build the declarations and the Stage 1 seam checks now; defer the sandbox until third-party code needs it — and never let third-party code precede it.
...
The **Civic Process Descriptor** (defined in the [Civic Process Specification](../../specs/civic-process-spec.md) section 6.1) is a JSON object describing what a process *is*: its type, title, jurisdiction, lifecycle configuration, action schemas, and eligibility requirements. Every process publishes its descriptor at `GET /process/:id`; the hub's full process catalog is available at `GET /process`. This is the static, structural side of the export.

The **Civic Activity Feed** (defined in the [Civic Activity Specification](../../specs/civic-activity-spec.md)) is a stream of JSON activity objects covering every action and every lifecycle transition that occurred — votes, comments, proposals, process state changes — each with attribution, timestamps, and a namespaced payload. The feed is available at `GET /events`, filterable by `process_id` for per-process slices. This is the dynamic, participation-record side of the export.
...
- **Stage 2 sandbox runtime as the enforcement mechanism.** Stage 1 host-seam enforcement (Civic Plugin Architecture §5a) — emission validation against the plugin's declared activity types and identity-claim filtering at handoff — **is in scope** for this pilot. What is deferred is Stage 2: the sandbox runtime that makes capability grants mechanically enforceable for Tier 2 code (e.g. blocking a plugin's network call to an undeclared domain). Per the plugin architecture's normative production boundary, no third-party plugin code runs in production before Stage 2 exists for its tier.
...
A self-hosted Polis instance is already deployed live at `polis.civic.social` and integrated into the representative-space service. The lessons of that first integration are codified as the Tier-3 external-service contract in the [Civic Plugin Architecture](../../ecosystem/civic-plugin-architecture-spec.md) §10a. The pilot will productize the integration as the first Tier 3 plugin under the full Civic Process Plugin Specification, exercising that contract end to end:

- per-participant identity handoff across the identity seam (Civic Identity ↔ Polis XID — an opaque, stable, per-participant identifier; never a shared service-account token standing in for participants)
- the activity-emission seam (Polis conversation events ↔ Civic Activities, through the host's single emission path)
- host lifecycle sovereignty (an unavailable external service must not wedge a host lifecycle transition) and host-provided scheduling (no assumed background scheduler)
- the anti-corruption adapter — exactly one host-side module speaks the Polis API, so the plugin handler stays service-agnostic
- the capability declaration model for external-service plugins (network access, identity claims, UI surfaces for the embedded conversation) and the data-ownership declaration (what participant data the external service retains, and its export path)
...
**Risk: capability declarations without enforcement create false confidence.** A plugin author or a host operator might assume the system is enforcing capability boundaries when only Stage 1 host-seam checks exist. *Mitigation: this is now a normative requirement of the plugin architecture rather than a documentation promise — the Civic Plugin Architecture §5a states that a host MUST NOT run third-party plugin code in production civic processes before Stage 2 sandbox enforcement exists for that code's tier. The pilot additionally documents the staged enforcement path prominently in the plugin specification and the host operator guide and marks the Stage 2 gap explicitly in conformance criteria.*
