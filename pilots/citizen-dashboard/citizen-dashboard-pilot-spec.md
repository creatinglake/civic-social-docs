...
- **[Civic Space Specification](../../specs/civic-space-spec.md)** — the scoped host contract every space conforms to. The dashboard is the individual-scoped space; §1.3–1.6 (scope taxonomy), §7 (Space API Profile), and §9.4 (individual portability profile) are the sections this pilot exercises most directly.
- **[Civic Activity Specification](../../specs/civic-activity-spec.md)** — the activity envelope and stream the dashboard consumes. For v0.1 the wire-format field is `event_type` and the transport endpoint is `GET /events`.
- **[Civic Process Specification](../../specs/civic-process-spec.md)** — the process descriptors and action contracts the dashboard participates against.
