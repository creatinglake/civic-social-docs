...
**Multi-tenant provisioning.** The ecosystem architecture audit (July 2026) laid out the SaaS path for spaces: row-level tenancy, a provisioning flow that creates the space, mints its DID, binds its founding admin, and serves its per-space discovery manifest. The audit describes that flow as the self-serve "spin up your own space" product, and notes that SaaS provisioning and the Civic Space primitive are one design: creating a hosted tenant and instantiating a Civic Space per the specification require the same machinery (an identity, a holder binding, a manifest, a host contract), so building them as separate efforts would mean building the same thing twice. That same flow is also where the builder's output goes live: when the builder finishes generating a space or process, it hands the result to this provisioning flow — the one that creates the space, mints its DID, binds its admin, and serves its manifest — exactly as if a person had clicked "create my space." There is no separate deployment path for generated artifacts, so a generated space is never a second-class citizen running on different infrastructure.
...
**The production boundary.** The [Civic Plugin Architecture](civic-plugin-architecture-spec.md) is unambiguous: a host must not run third-party plugin code in production civic processes before sandbox (Tier 2) enforcement exists for that code's tier. The reasoning bears spelling out. Code that runs inside the host's own program (Tier 1) physically has access to everything the program has — the database, participant identity context, other processes' state — and nothing constrains it; it is safe only because the host operator wrote and vetted it, a posture that cannot extend to strangers. The Tier 2 WebAssembly sandbox is what makes untrusted code safe to run anyway: a sandboxed plugin is a sealed box with no ambient access to network, storage, or memory beyond its own, able to call only the specific host functions the runtime installs through the wall of the box. Inside the sandbox a capability grant is not a promise the plugin makes but a physical fact — if network egress is not granted, no network function exists for the plugin to call. The sandbox has been deliberately deferred because early plugins are all first-party; building the enforcement machinery before any third-party code exists would be premature. A builder that generates code from public prompts ends that deferral, because it is a machine for producing third-party code — anyone with a prompt becomes an unvetted plugin author — and its output must be treated with exactly that trust posture regardless of the conformance suite passing. The consequence is direct: **the Tier 2 WebAssembly sandbox becomes a hard prerequisite of the hosted builder.** Generated processes should target the sandbox by construction, or run as externally isolated (Tier 3) services. This concept is, in effect, the strongest known argument for scheduling the sandbox work.
