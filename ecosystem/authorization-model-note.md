...
| **Activity** | No authorization at all — only its residue. An activity carries its audience as standard AS2 addressing (public activities address `as:Public`; restricted ones address space-managed collections), the outcome of a decision already made; rules never travel on activities, and the emitting space's seam decides which callers a restricted audience resolves to. | Civic Activity Specification §5 |
